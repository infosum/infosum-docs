...
For a detailed description and information on Transformations, you can review the specific page [here](../transform).

<Authentication/>

## Listing files

If no files have been uploaded to the [Cloud Vault](../cloudvaults), please refer to the getting started guides for [uploading a file from AWS S3](./uploading-a-file-from-aws-s3-to-a-cloudvault).
...
This will give you a list of available files within the Cloud Vault and the associated meta data.
...
Let's assume that the data that needs transforming is in the form of a CSV file, as shown:
...
How can you transform these boolean columns into a simpler format of a `userID` with just the associated `cat_*`?
The following bloblang script does this. By declaring the `input` file and how to read it, the `bloblang` itself as a transformation pipeline, and the `output` for the format to return to the Cloud Vault.
...
For more details on `bloblang`, please refer to our [Benthos Bloblang documentation](https://www.benthos.dev/docs/guides/bloblang/about).
...
To do this, you will need to encode your `bloblang` script into the `script` element as shown in the following API request:
...
Once the `bloblang` script has passed validation, you can execute it against your files. You can run either a `scaled` or `static` transformation task.
