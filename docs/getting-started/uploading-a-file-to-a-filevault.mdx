...
For specific information on [File Vaults](/docs/filevaults/) we have a dedication page [here](/docs/filevaults/).
...
Once you have a File Vault you will need run an import process from your storage location to the File Vault.

### Create an Import Connector

First step is to setup an `Import Connector Config` (or `ICC` for short), to manage the connection between your AWS S3 or Google GCS and the InfoSum Platform.
...
- An S3 connection object in `s3_connector`
    - The name of the AWS S3 bucket as `bucket`
    ...
    - An authentication object under `cross_account` with
        ...
        - The name of the session as `session_name`
